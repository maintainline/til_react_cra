...
- falshy 한 값 종류 : `false, null, undefinde, 0, "", NanN`
- if 문
...
- Null 병함 연산자(`??`) : null 또는 undefined 일때만 기본 값 사용

```js
const user = { age: 10, job: "개발자" };
const result = user.gogo ?? "없어용";
```
...
- JSX 에서는 `JS자리`에 if 문과 switch 문을 사용할 수 있음.

- JSX 에서는 `JSX자리`에 if 문과 switch 문을 사용할 수 없습니다.
- if 문과 switch 문 대신에 `3항 연산자`가 가장 많이 사용됨

- JSX 에서는 `JSX자리`에 falshy 한 값은 출력이 안됩니다..
...
function Hi({ isLogin }) {
  ...
  //jsx
  return <div>{isLogin ? "하이염" : "로그인하세요."}</div>;
}
...
- `객체?. 속성명`
- 객체가 null 이거나 undefined 이면 리액트 오류남.
- 복잡한 if 문 대신 옵셔널 체이닝 활용함.
...
function Hi({ isLogin, msg, user }) {
  ...
  return (
    <div>
      ...
      <div>{msg && "메세지가 있습니다."}</div>
      <div>
        {user?.name}:{user?.age ? user?.age : "나이가 없어용"}
      </div>
      <div>
        {user?.name}:{user?.age || "나이가 없어용"}
      </div>
    </div>
  );
}
...
function Test() {
  ...
  // jsx
  return (
    <Hi isLogin={true} msg={"new"} user={{ name: "홍길동" }}>
      Test
    </Hi>
  );
}
